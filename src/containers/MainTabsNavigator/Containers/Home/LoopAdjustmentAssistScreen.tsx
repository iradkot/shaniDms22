...
const LoopAdjustmentAssistScreen: React.FC<any> = ({route}) => {
  ...
  const generateRecommendation = async () => {
    ...
    try {
      ...
      const iobAroundEarlyLows = overnightEarlyLows.map(low => {
        const near = dsSeries
      ...
      const iobAroundPredawnLows = predawnLows.map(low => {
        const near = dsSeries
          .filter(d => Math.abs(Number(d.ts) - low.ts) <= 25 * 60 * 1000)
          .sort((a, b) => Math.abs(Number(a.ts) - low.ts) - Math.abs(Number(b.ts) - low.ts))[0];
        return near?.iob ?? null;
      ...
      const autosensValues = dsSeries.map(d => d.autosensRatio).filter((n): n is number => Number.isFinite(n));
      ...
      const contextForAi = {
        ...
        loopDiagnostics: {
          ...
          interpretationHints: {
            earlyLowWithHighIobLikely: 'evening CR too aggressive or late bolus overlap, not necessarily night target',
